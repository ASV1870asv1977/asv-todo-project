...
class TestProjectViewSet(TestCase):
    ...
    def test_live_create_project(self):
        ...
        action = ['users', 'list']
        data = client.action(schema, action)
        users = random.choices(data['results'], k=random.choice(range(len(data['results']))))
        # print(users)

        params = {
            'name': 'liveTestProjectCreate' + str(random.randint(0, 10000)),
            'users': [user['id'] for user in users],
            'url_repo': ''
        }
        action = ['projects', 'create']
        data_response = client.action(schema, action, params, validate=True)
        # print(data_response)
        self.assertEqual(type(data_response.get('id')), int)
